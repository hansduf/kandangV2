-- ==============================================================================
-- UPDATE V12: MULTI-FLOCK, MULTI-WORKER, DATE RANGE & UNCOMPLETED COOP SYNC
-- ==============================================================================
-- 1. Menambahkan kolom flock_ids (UUID[]) dan assigned_to_ids (UUID[]) pada farm_tasks
-- 2. Memperbaiki logika get_tasks_for_date agar:
--    - Tugas hanya selesai jika SEMUA kandang yang dipilih (flock_ids atau seluruh kandang aktif) sudah selesai
--    - Mendukung filter banyak pekerja (assigned_to_ids)
--    - Mendukung rentang tanggal start_date s/d end_date
-- ==============================================================================

-- 1. Tambah kolom flock_ids dan assigned_to_ids jika belum ada
ALTER TABLE public.farm_tasks
ADD COLUMN IF NOT EXISTS flock_ids UUID[],
...
-- 3. Buat ulang fungsi get_tasks_for_date dengan dukungan multi-kandang & multi-pekerja
